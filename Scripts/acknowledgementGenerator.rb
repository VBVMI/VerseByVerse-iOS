...
unless ARGV.length == 3
  abort("Not the right number of arguments.\n" +
        "Usage: ruby acknowledgementGenerator.rb \"path/to/Settings.bundle\" \"path/to/licenses/\" \"path/to/pods/\"")
end

# Constants
SETTINGS_BUNDLE = ARGV[0]
LICENSE_FILE_DIR_PATH = ARGV[1]
POD_LICENSE_FILE_DIR_PATH = "#{LICENSE_FILE_DIR_PATH}/Pods"
POD_DIR_PATH = ARGV[2]
...
if __FILE__ == $PROGRAM_NAME
    ...
    # Remove the old acknowledgements
    Dir.glob("#{SETTINGS_BUNDLE}/Acknowledgement-*") do |file|
      FileUtils.rm(file)
    end

    # Find all of the licenses in the pods directory
    puts "Searching Pods for licenses"
    count = 0
    FileUtils.mkdir_p POD_LICENSE_FILE_DIR_PATH
    FileUtils.remove_dir POD_LICENSE_FILE_DIR_PATH
    FileUtils.mkdir_p POD_LICENSE_FILE_DIR_PATH

    Dir.glob("#{POD_DIR_PATH}/*/[lL][iI][cC][eE][nN][sS][eE]*") do |file|
      components = file.split('/')
      projectName = components[components.length - 2]
      destination = "#{POD_LICENSE_FILE_DIR_PATH}/#{projectName}#{LICENSE_FILE_EXTENSION}"
      puts "Moving: #{projectName}/#{components.last} to #{destination}"
      FileUtils.cp(file, destination)
      count = count + 1
    end
    puts "Found #{count} licenses"
